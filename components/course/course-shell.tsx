...
export function CourseShell({
  ...
  return (
    <div className="flex h-full min-h-0 flex-1 flex-col bg-white">
      <div className="flex h-12 shrink-0 items-center gap-2 border-b border-[#e8eaec] px-3 md:px-4">
        <Button
          variant="ghost"
          size="icon"
          className="lg:hidden"
          ...
          {breadcrumbs.map((crumb, index) => {
            ...
            return (
              <span key={`${crumb.label}-${index}`} className="flex min-w-0 items-center gap-1.5">
                {index > 0 && <span className="text-[#6b7780]">&gt;</span>}
                {crumb.href && !isLast ? (
                  <Link
                    href={crumb.href}
      ...
      <div className="flex min-h-0 flex-1">
        ...
        <Sheet open={navOpen} onOpenChange={setNavOpen}>
          <SheetContent
            side="left"
            className="w-[220px] gap-0 p-0 sm:max-w-[220px]"
            showCloseButton
          >
